...
| Bind address | `127.0.0.1` | `127.0.0.1` (local only) or `0.0.0.0` (LAN). `0.0.0.0` triggers a warning. *(requires restart)* |
...
| Setting (UI label) | Default | Notes |
|---|---|---|
| Enable MCP server | on | Starts the HTTP server. The always-on tiers (`read`, `writeScoped`, `agent`) become available once the server is running. They are not individually toggleable. |
