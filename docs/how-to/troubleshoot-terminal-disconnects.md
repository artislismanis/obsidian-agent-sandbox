...
|---|---|---|
| `1006` abnormal-no-close-frame, `idleMsBeforeClose` high | `WS client disconnected` around the same time, no errors before | Idle network path: WSL2 vNAT, host firewall, or VPN closed the socket. The auto-reconnect recovers it. |
...
| `1000`/`1001`/`1005` (clean) | Paired `[oas-session] end` with `exit=0` | Container was stopped or the tab was closed on purpose. No reconnect: the error screen prompts a manual retry. |
...
- **Check port conflicts.** `ss -ltn '( sport = :7681 )'` (Linux) or the Obsidian status-bar tooltip. Another process listening on `ttydPort` makes new ttyd connections fail.
- **Check the host firewall** (your OS firewall, not the container's outbound firewall). On Windows + WSL2 with mirrored networking, Windows Defender Firewall can block loopback traffic to the container's published port. The [container outbound firewall doc](configure-firewall.md) covers a different layer (container → internet); host-firewall issues block the plugin's WebSocket from reaching ttyd in the first place.
