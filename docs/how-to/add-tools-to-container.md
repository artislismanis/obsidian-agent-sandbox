...
The container's sudoers entry permits `apt-get` and `apt` only, password-gated on the `sudoPassword` plugin setting (default empty: sudo is disabled until you set a password in **Settings → Agent Sandbox → Advanced → Sudo password**, or via `OAS_SUDO_PASSWORD` in `container/.env`). You can't `sudo su` or run arbitrary commands.
